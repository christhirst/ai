...
pub trait DbDeletable {
    ...
    fn delete_gdps_by_year(
        &self,
        year: &str,
    ) -> impl std::future::Future<Output = Result<Vec<GdpRecord>, Box<dyn std::error::Error>>> + Send;
    fn delete_all_homecides(
        &self,
    ) -> impl std::future::Future<Output = Result<Vec<Homecides>, Box<dyn std::error::Error>>> + Send;
    fn delete_homecides_by_city(
        &self,
        city: &str,
    ) -> impl std::future::Future<Output = Result<Vec<Homecides>, Box<dyn std::error::Error>>> + Send;
    fn delete_homecides_by_citizenship(
        &self,
        citizenship: &str,
    ) -> impl std::future::Future<Output = Result<Vec<Homecides>, Box<dyn std::error::Error>>> + Send;
}
...
impl<C: Connection + Send + Sync> DbDeletable for Surreal<C> {
    ...
    async fn delete_homecides_by_city(
        &self,
        city: &str,
    ...
    async fn delete_homecides_by_citizenship(
        &self,
        citizenship: &str,
    ) -> Result<Vec<Homecides>, Box<dyn std::error::Error>> {
        let mut response = self
...
impl DbDeletable for AppDb {
    async fn delete_all_gdps(&self) -> Result<Vec<GdpRecord>, Box<dyn std::error::Error>> {
        match self {
            ...
            AppDb::Remote(remote) => {
                let res = remote
                    .query_raw("DELETE FROM gdp_record RETURN BEFORE")
                    .await?;
    ...
    async fn delete_all_homecides(&self) -> Result<Vec<Homecides>, Box<dyn std::error::Error>> {
        match self {
            ...
            AppDb::Remote(remote) => {
                let res = remote
                    .query_raw("DELETE FROM homecides RETURN BEFORE")
                    .await?;
    ...
    async fn delete_homecides_by_citizenship(
        ...
        match self {
            ...
            AppDb::Remote(remote) => {
                ...
                let sql =
                    format!("DELETE FROM homecides WHERE Citizenship = '{escaped}' RETURN BEFORE");
                let res = remote.query_raw(&sql).await?;
                let records: Vec<Homecides> = serde_json::from_value(res).unwrap_or_default();
                Ok(records)
            }
...
pub async fn delete_all_gdp_records<T: DbDeletable>(
    db: &T,
) -> Result<Vec<GdpRecord>, Box<dyn std::error::Error>> {
...
pub async fn delete_all_homecides<T: DbDeletable>(
    db: &T,
) -> Result<Vec<Homecides>, Box<dyn std::error::Error>> {
