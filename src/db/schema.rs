...
pub trait SchemaIntrospectable {
    fn db_info(
        &self,
    ) -> impl std::future::Future<Output = Result<serde_json::Value, Box<dyn std::error::Error>>> + Send;
    fn table_info(
        &self,
        table: &str,
    ) -> impl std::future::Future<Output = Result<serde_json::Value, Box<dyn std::error::Error>>> + Send;
    fn defined_tables(
        &self,
    ) -> impl std::future::Future<Output = Result<Vec<String>, Box<dyn std::error::Error>>> + Send;
    fn execute_query(
        &self,
        sql: &str,
    ) -> impl std::future::Future<Output = Result<serde_json::Value, Box<dyn std::error::Error>>> + Send;
...
pub async fn get_db_info<T: SchemaIntrospectable>(
    db: &T,
) -> Result<serde_json::Value, Box<dyn std::error::Error>> {
    db.db_info().await
}
...
pub async fn get_defined_tables<T: SchemaIntrospectable>(
    db: &T,
...
pub async fn get_table_schema_summary<T: SchemaIntrospectable>(
    ...
    if let Some(tables_obj) = db_info.get("tables").and_then(|t| t.as_object())
        && let Some(tbl_def) = tables_obj.get(table)
    {
